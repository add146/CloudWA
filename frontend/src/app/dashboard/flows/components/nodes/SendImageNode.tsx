...
export const SendImageNode = memo(({ id, data, selected }: NodeProps) => {
    ...
    return (
        <div className={`w-[250px] bg-white rounded-lg border shadow-sm ${selected ? 'border-violet-500 ring-1 ring-violet-200' : 'border-violet-200'}`}>
            {/* Header */}
            <div className="flex items-center gap-2 p-3 border-b border-violet-100 bg-violet-50/50 rounded-t-lg">
                <div className="p-1.5 rounded bg-violet-100 text-violet-600">
                    ...
                    {isUploading ? (
                        ...
                        <>
                            ...
                            <span className="text-xs text-center text-gray-500">
                                {data.fileName ? (data.fileName as string) : "Click to upload an image"}
                            </span>
                        </>
                    ))}
                </div>
